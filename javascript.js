...
class Usuario {
  ...
  filtarDireccion() {
    return `C/ ${this.#direccion.calle}, ${this.#direccion.ciudad} (${
      this.#direccion.codigoPostal
    }).`;
  }
}
...
for (let i = 0; i < ciudadesSinRepetir.length - 1; i++) {
  for (let j = i + 1; j < ciudadesSinRepetir.length; j++) {
    if (
      Usuario.getCiudad(ciudadesSinRepetir[i]) ==
      Usuario.getCiudad(ciudadesSinRepetir[j])
    ) {
...
for (let i = 0; i < ciudadesSinRepetir.length; i++) {
  ...
  for (let j = 0; j < nombreMapeado.length; j++) {
    if (
      Usuario.getCiudad(nombreMapeado[i]) == Usuario.getCiudad(nombreMapeado[j])
    ) {
...
creacionModal.insertAdjacentHTML(
  "beforebegin",
  `
...
function calcularDatos(listaUsuarios) {
  ...
  return (
    "El usuario mayor de la lista es " +
    Usuario.getNombre(mayor) +
    " y el usuario menor es " +
    Usuario.getNombre(menor) +
    " hay " +
    menoresDeEdad +
    " menores de edad y " +
    mayoresDeEdad +
...
document.getElementById("listaUsuarios").insertAdjacentHTML(
  "beforeend",
  `
    <div>
...
nombreMapeado.forEach((ele) => {
  document
    .getElementById("selectorUsuarios")
    .insertAdjacentHTML(
      "beforeend",
...
document.getElementById("selectorUsuarios").addEventListener("change", () => {
  ...
  document.getElementById("direccionUsuario").textContent =
    usuarioSeleccionado.filtarDireccion();
});
